 ...
 */
 ...
 import { CustomBlurMaterial } from '../../materials/CustomBlurMaterial.js'
...
export class Reflector extends Group {
    constructor({
        ...
        // Render targets
        this.renderTarget = new WebGLRenderTarget(width / this.reductionRate, height / this.reductionRate, {
            //format: RGBFormat,
            format: RGBAFormat,
            depthBuffer: false
        });

        this.renderTargetRead = new WebGLRenderTarget(Math.ceil(width / this.reductionRate), Math.ceil(height/this.reductionRate), {
          format: RGBAFormat,
          depthBuffer: false
        });
        ...
        //this.blurMaterial.uniforms.uBluriness.value = 0.2
        //this.blurMaterial.uniforms.uResolution.value.set(width, height);

        this.blurMaterial = new CustomBlurMaterial();
        this.blurMaterial.uniforms.u_sampleStep.velue = 1.0;
    ...
    setSize(width, height) {
        ...
        this.renderTarget.setSize(width / this.reductionRate, height / this.reductionRate);
        this.renderTargetRead.setSize(width / this.reductionRate, height / this.reductionRate);
        this.renderTargetWrite.setSize(width / this.reductionRate, height / this.reductionRate);
        //this.blurMaterial.uniforms.uResolution.value.set(width, height);
    }

    setReductionRate(_reductionRate){
      
      this.reductionRate = _reductionRate;
      this.renderTarget.setSize(this.width / this.reductionRate, this.height / this.reductionRate);
      this.renderTargetRead.setSize(this.width / this.reductionRate, this.height / this.reductionRate);
      this.renderTargetWrite.setSize(this.width / this.reductionRate, this.height / this.reductionRate);

       // Render targets
      /*
       this.renderTarget = new WebGLRenderTarget(this.width / this.reductionRate, this.height / this.reductionRate, {
            //format: RGBFormat,
            format: RGBAFormat,
            depthBuffer: false
        });
    ...
    setSampleStep(_step){
      this.blurMaterial.uniforms.u_sampleStep.value = _step;
    }
    ...
    update(renderer, scene, camera) {
        ...
        for (let i = 0; i < blurIterations; i++) {
            ...
            // const radius = blurIterations

          /*this.blurMaterial.uniforms.uDirection.value.set(
                i % 2 === 0 ? radius : 0,
                i % 2 === 0 ? 0 : radius
            );*/
            this.blurMaterial.uniforms.u_horizontal.value =  ( i%2 === 0 ? true : false )
